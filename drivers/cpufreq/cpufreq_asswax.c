...
static void cpufreq_asswax_timer(unsigned long cpu)
{
	...
	delta_idle = cputime64_sub(now_idle, this_asswax->time_in_idle);
	delta_time = cputime64_sub(update_time, this_asswax->idle_exit_time);
	...
	if (cpu_load > max_cpu_load || delta_idle == 0)
	{
		if (old_freq < policy->max &&
			 (old_freq < this_asswax->ideal_speed || delta_idle == 0 ||
			  cputime64_sub(update_time, this_asswax->freq_change_time) >= up_rate_us))
		{
	...
	// frequency we require that we have been at this frequency for at least down_rate_us:
		 ...
		 (old_freq > this_asswax->ideal_speed ||
		  cputime64_sub(update_time, this_asswax->freq_change_time) >= down_rate_us))
	{
